...
namespace Beer.DaAPI.UnitTests.Core.Scopes.DHCPv6
{
    public class DHCPv6RootScopeTester
    {
        ...
        public void DeleteScope_NonRootScope_IncludeChildren()
        {
            ...
            var changes = rootScope.GetChanges();
            Assert.Equal(1 + subChildAmount, changes.Count());
            for (int i = 0; i < subChildAmount; i++)
            {
                ...
                subChildIds.Remove(@event.EntityId);
            }

            Assert.Empty(subChildIds);
            Assert.Equal(scopeId, (changes.Last() as DHCPv6ScopeDeletedEvent).EntityId);
        }
        ...
        public void DeleteScope_RootScope_IncludeChildren()
        {
            ...
            var changes = rootScope.GetChanges();
            Assert.Equal(2 + subChildAmount, changes.Count());
            for (int i = 0; i < subChildAmount; i++)
            {
                DHCPv6ScopeDeletedEvent @event = changes.ElementAt(i) as DHCPv6ScopeDeletedEvent;
            ...
            Assert.Empty(subChildIds);
            Assert.Equal(scopeId, (changes.ElementAt(subChildAmount) as DHCPv6ScopeDeletedEvent).EntityId);
            Assert.Equal(parentId, (changes.Last() as DHCPv6ScopeDeletedEvent).EntityId);
        }

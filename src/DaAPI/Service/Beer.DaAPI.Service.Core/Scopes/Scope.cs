...
namespace Beer.DaAPI.Core.Scopes
{
        ...
        where TScopeProperty : ScopeProperty<TOption, TValueType>
    {
        ...
        internal void AttachAllScopesAndLeases(IDictionary<Guid, IEnumerable<Guid>> scopeAndLeaseDict, Boolean includeChildren)
        {
            if (includeChildren == true)
            {
                foreach (var item in _subscopes)
                {
                    item.AttachAllScopesAndLeases(scopeAndLeaseDict,true);
                }
            }

            scopeAndLeaseDict.Add(this.Id, Leases.GetAllLeases().Select(x => x.Id));
        }
    }
}

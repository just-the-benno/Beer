...
namespace Beer.DaAPI.Core.Scopes
{
        ...
        where TLease : Lease<TLease, TAddress>
    {
        ...
        internal IEnumerable<Guid> DropUnusedLeasesOlderThan(DateTime threshold) => 
            Entries.Where(x => x.Value.IsActive() == false && x.Value.End < threshold).Select(x => x.Key).ToList();
